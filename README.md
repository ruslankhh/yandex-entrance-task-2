...
- Сделал инпуты для даты и времени через стандартные типы `date` и `time`. В силу чего немного изменились пропорции инпутов, чтобы влезли стандартные интерфейсы.
...
- Сверстал все компоненты, кроме calendar, datepicker и input-select.
