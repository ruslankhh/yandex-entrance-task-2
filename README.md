...
- [x] Выпадающий календарь для поля выбора даты делать необязательно, но это будет плюсом.
- [x] Участники встречи выбираются из списка, который появляется, когда пользователь переходит на поле поиска сотрудника.
...
- На странице создания встречи кнопки выбора переговорок реализовал через инпут типа `checkbox`.
- Изображения emoji в виде SVG с текстом этого emoji выглядят забавно)
- Сделал инпуты для даты и времени через стандартные типы `date` и `time`. В силу чего немного изменились пропорции инпутов, чтобы влезли стандартные интерфейсы. К сожалению, это не поддерживается в Safari.
- Сделал инпут со списком выбора через стандартные `input` и `datalist`. К сожалению, это не поддерживается в Safari.
- Так как я все инпуты сделал через стандартные интерфейсы (в силу нехватки времени), я решил убрать все кастомные иконки и кнопки из инпутов.
...
После того, как начал делать третье задание, появились следующие правки:
...
- [ ] Вынести элементы `app-header` и `app-footer` в `app`.
- [ ] Сделать элемент `app-header` фиксированным, а у `app-main` сделать `padding-top`.
- [ ] Убрать элементы `<form>`, мы не отправляем запросы через форму.
- [ ] В блоке `button-checkbox` сделать минимальную высоту, чтобы он не схлопывался, если нет данных.
- [ ] В блоках `input` и `input-datalist` сделать `padding` вместо `margin` лейблу, чтобы он не моргал при наведении.
- [ ] В блоке `room-item` проверить отступы у `text-separator`.
- [ ] У блока `user-item` убрать минимальную ширину.
- [ ] Сделать `padding-left` у `user-item__content`.
- [ ] Все блоки `*-group` убрать из миксинов или переименовать в зависимости от специфики.
- [ ] Убрать следы блока `timetable`.
- [ ] Сделать элемент `body__sidebar-body` на всю доступную высоту.
